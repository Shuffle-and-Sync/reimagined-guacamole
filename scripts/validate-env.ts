...
function printValidationResults() {
  ...
  console.log(`\n📊 Validation Results:`);
  console.log(
    `├── Status: ${result.isValid ? colorize("✅ PASSED", "green") : colorize("❌ FAILED", "red")}`,
  );
...
function printVariableDefinitions() {
  ...
  const requiredVars =
    nodeEnv === "production"
      ? defs.required.production
      : defs.required.development;

  console.log(colorize(`\n🔴 Required (${nodeEnv}):`, "red"));
  requiredVars.forEach((varName) => {
    const hasValidation = defs.hasValidation.includes(varName)
      ? " (validated)"
      : "";
...
function printHelp() {
  ...
  console.log(
    "  tsx scripts/validate-env.ts --definitions Show variable definitions",
  );
...
function main() {
  ...
  if (!result.isValid) {
    console.log(
      colorize(
        "\n💡 Tip: Check .env.example for detailed setup instructions",
        "cyan",
      ),
    );
    process.exit(1);
