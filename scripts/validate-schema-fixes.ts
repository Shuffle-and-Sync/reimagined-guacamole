...
const missingImportantSchemas = importantTablesForValidation.filter((table) => {
  ...
  if (table === "tournamentParticipants")
    return !("insertTournamentParticipantSchema" in schema);
  if (table === "friendships") return !("insertFriendshipSchema" in schema);
  if (table === "streamSessions")
    return !("insertStreamSessionSchema" in schema);
...
if (missingImportantSchemas.length > 0) {
  console.log(
    `⚠️  ${missingImportantSchemas.length} important tables without insert schemas (non-critical)`,
  );
