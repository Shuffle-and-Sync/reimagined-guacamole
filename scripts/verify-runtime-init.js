...
try {
  ...
  if (
    ...
    envModule.validateAndLogEnvironment
  ) {
...
try {
  const monitoringModule = await import(
    `${projectRoot}/server/services/monitoring-service`
  );
  ...
  console.log(
    "   ⚠️  Monitoring service not available (optional):",
    error.message,
  );
}
...
try {
  const communitiesModule = await import(
    `${projectRoot}/server/features/communities/communities.routes`
  );
  const eventsModule = await import(
    `${projectRoot}/server/features/events/events.routes`
  );
  const usersModule = await import(
    `${projectRoot}/server/features/users/users.routes`
  );

  if (
    communitiesModule.communitiesRoutes &&
