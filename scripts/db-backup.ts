  ...
  try {
    ...
    if (databaseUrl.startsWith("sqlitecloud://")) {
      ...
      try {
        ...
        console.log(
          "⚠️  SQLite Cloud CLI not available. Using alternative export method...",
        );
        // Alternative: Use Drizzle to export data
        console.log(
          "Note: Consider installing SQLite Cloud CLI for better backup capabilities",
        );
    ...
    if (options.uploadGcs) {
      ...
      try {
        ...
        console.error(
          "   Install Google Cloud SDK and authenticate to enable GCS upload",
        );
      }
    }
