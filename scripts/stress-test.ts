...
class StressTester {
  ...
  async runTest(): Promise<void> {
    ...
    console.log(
      `Users: ${config.startUsers} → ${config.maxUsers} (increment by ${config.userIncrement} every ${config.incrementInterval}s)`,
    );
    ...
    while (elapsed < testDuration && this.activeUsers <= config.maxUsers) {
      ...
      if (phaseResults.length > 0) {
        const phaseResult: StressPhaseResult = {
          ...
          averageResponseTime:
            phaseResults.reduce((a, b) => a + b, 0) / phaseResults.length,
        ...
        console.log(
          `  Requests: ${phaseResult.totalRequests} (${phaseResult.successfulRequests} success, ${phaseResult.failedRequests} failed)`,
        );
        console.log(
          `  Avg Response: ${phaseResult.averageResponseTime.toFixed(2)}ms`,
        );
        console.log(
          `  P95 Response: ${phaseResult.p95ResponseTime.toFixed(2)}ms`,
        );
        console.log(`  Error Rate: ${phaseResult.errorRate.toFixed(2)}%`);
        console.log(
          `  Throughput: ${phaseResult.requestsPerSecond.toFixed(2)} req/s`,
        );
      }
    ...
    while (this.isRunning) {
      ...
      if (progress < 100) {
        process.stdout.write(
          `\rProgress: ${progress.toFixed(1)}% | Users: ${this.activeUsers} | Requests: ${this.results.length}`,
        );
      }
    }
  }
  ...
  private printResults(): void {
    ...
    this.phaseResults.forEach((phase) => {
      ...
      console.log(
        `  Success Rate: ${((phase.successfulRequests / phase.totalRequests) * 100).toFixed(2)}%`,
      );
    ...
    const avgResponseTime =
      this.results.reduce((a, b) => a + b, 0) / totalRequests;

    console.log("Overall Summary:");
    console.log("=".repeat(60));
    console.log(`Total Phases: ${this.phaseResults.length}`);
    console.log(
      `Max Concurrent Users: ${Math.max(...this.phaseResults.map((p) => p.concurrentUsers))}`,
    );
    console.log(`Total Requests: ${totalRequests}`);
    console.log(`Success Rate: ${successRate.toFixed(2)}%`);
    console.log(`Average Response Time: ${avgResponseTime.toFixed(2)}ms`);
    console.log(
      `P95 Response Time: ${this.calculatePercentile(this.results, 95).toFixed(2)}ms`,
    );
    console.log(
      `P99 Response Time: ${this.calculatePercentile(this.results, 99).toFixed(2)}ms`,
    );

    // Find breaking point
    const breakingPhase = this.phaseResults.find(
      (p) => p.errorRate > 5 || p.averageResponseTime > 2000,
    );
    ...
    if (breakingPhase) {
      ...
      console.log(
        `  Avg Response Time: ${breakingPhase.averageResponseTime.toFixed(2)}ms`,
      );
    } else {
      console.log(
