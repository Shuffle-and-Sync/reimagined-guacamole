...
class LoadTester {
  ...
  private errors: Map<string, Array<{ status: number; message: string }>> = new Map();
  ...
  private printResults(): void {
    ...
    allResults.forEach(result => {
      ...
      console.log(`  Success: ${result.successfulRequests} (${((result.successfulRequests / result.totalRequests) * 100).toFixed(2)}%)`);
    ...
    const totalSuccess = allResults.reduce((sum, r) => sum + r.successfulRequests, 0);
    const totalFailed = allResults.reduce((sum, r) => sum + r.failedRequests, 0);
    const overallAvgTime = allResults.reduce((sum, r) => sum + (r.averageResponseTime * r.totalRequests), 0) / totalRequests;
    
    console.log('Overall Summary');
    console.log('='.repeat(60));
    console.log(`Total Requests: ${totalRequests}`);
    console.log(`Success Rate: ${((totalSuccess / totalRequests) * 100).toFixed(2)}%`);
    console.log(`Failure Rate: ${((totalFailed / totalRequests) * 100).toFixed(2)}%`);
    ...
    if (testPassed) {
      ...
      console.log('\n❌ Load test FAILED');
      if (successRate < 95) {
        console.log(`  - Success rate ${successRate.toFixed(2)}% is below 95% threshold`);
      }
      if (!avgResponseTimeAcceptable) {
        console.log(`  - Average response time ${overallAvgTime.toFixed(2)}ms exceeds 500ms threshold`);
      }
    }
  }
}
