...
class IssuePRHistoryAgent {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      if (!this.githubToken) {
        ...
        console.log('   The agent will run in limited mode without GitHub API access');
        console.log('   To enable full functionality, set GITHUB_TOKEN in your environment\n');
      }
      ...
      if (this.githubToken) {
        ...
        console.log(`   Found ${closedIssues.length} closed issues and ${closedPRs.length} closed PRs\n`);
      }

      // Update the document
      console.log('📝 Updating ISSUE_PR_HISTORY.md...');
      const updatedContent = await this.updateHistoryDocument(currentContent, closedIssues, closedPRs);
  ...
  private async fetchClosedIssues(): Promise<GitHubIssue[]> {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(`⚠️  GitHub API request failed: ${response.status} ${response.statusText}`);
        return [];
      }
  ...
  private categorizeIssue(issue: GitHubIssue): string {
    ...
    if (labels.some(l => l.includes('bug') || l.includes('error'))) {
      ...
      return 'Security';
    } else if (labels.some(l => l.includes('database') || l.includes('schema'))) {
      return 'Database & Schema';
    } else if (labels.some(l => l.includes('infrastructure') || l.includes('deployment'))) {
  ...
  private generateEntry(item: GitHubIssue | GitHubPullRequest): string {
    ...
    const closedDate = item.closed_at ? this.formatDate(item.closed_at) : 'Unknown';
