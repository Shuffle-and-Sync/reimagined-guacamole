    ...
    for (const envVar of Object.keys(process.env)) {
      if (
        ...
        if (value && weakPatterns.some((pattern) => value.includes(pattern))) {
          throw new Error(
            `Weak credential detected in ${envVar}. Please use secure credentials in production.`,
          );
        }
      }
    }
...
export function isSecureToken(token: string): boolean {
  ...
  const complexity = [hasUppercase, hasLowercase, hasNumbers].filter(
    Boolean,
  ).length;
...
export function auditSecurityConfiguration(): {
  ...
  try {
    ...
    if (process.env.NODE_ENV === "production") {
      ...
      logger.warn("Security audit found environment issues", {
