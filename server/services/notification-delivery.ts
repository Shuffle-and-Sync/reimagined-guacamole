...
export class NotificationDeliveryService {
  ...
  async deliverNotification(
    ...
    try {
      ...
      logger.error('Failed to deliver notification', { error, userId, notificationId: notification.id });
      throw error;
    }
  }
   ...
   */
  private async deliverBrowserNotification(userId: string, notification: Notification): Promise<DeliveryResult> {
    try {
   ...
   */
  private async deliverEmailNotification(user: User, notification: Notification): Promise<DeliveryResult> {
    try {
   ...
   */
  private async deliverPushNotification(userId: string, notification: Notification): Promise<DeliveryResult> {
    try {
   ...
   */
  private async deliverSMSNotification(user: User, notification: Notification): Promise<DeliveryResult> {
    try {
   ...
   */
  private async deliverWebhookNotification(user: User, notification: Notification): Promise<DeliveryResult> {
    try {
   ...
   */
  private getUserNotificationPreferences(userSettings?: UserSettings): NotificationPreferences {
    const defaultPreferences: NotificationPreferences = {
      streamStarted: { browser: true, email: false, push: true, sms: false, webhook: false },
      streamEnded: { browser: true, email: false, push: false, sms: false, webhook: false },
      collaborationInvite: { browser: true, email: true, push: true, sms: false, webhook: false },
      raidIncoming: { browser: true, email: false, push: true, sms: false, webhook: false },
      eventReminders: { browser: true, email: true, push: true, sms: false, webhook: false },
      friendRequests: { browser: true, email: true, push: false, sms: false, webhook: false },
      socialUpdates: { browser: false, email: false, push: false, sms: false, webhook: false },
      tournamentUpdates: { browser: true, email: true, push: true, sms: false, webhook: false },
      systemAnnouncements: { browser: true, email: true, push: false, sms: false, webhook: false },
      weeklyDigest: { browser: false, email: true, push: false, sms: false, webhook: false },
  ...
  private getChannelsForNotificationType(
    ...
    const typePreferences = preferences[notificationType as keyof NotificationPreferences] as NotificationChannels;
  ...
  registerWebSocketConnection(userId: string, connection: any): void {
    ...
    logger.info('WebSocket connection registered for notifications', { userId });
  }
  ...
  private async processEmailQueue(): Promise<void> {
    ...
    logger.info(`Processing ${this.emailQueue.length} queued email notifications`);
  ...
  private async processWebhookQueue(): Promise<void> {
    ...
    logger.info(`Processing ${this.webhookQueue.length} queued webhook notifications`);
