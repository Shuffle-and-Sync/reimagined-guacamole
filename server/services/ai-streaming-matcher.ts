...
export interface ContentPreferences {
  ...
  contentTypes: ('gameplay' | 'tutorial' | 'tournament' | 'casual' | 'speedrun' | 'review')[];
  collabTypes: ('co_op' | 'versus' | 'teaching' | 'tournament' | 'casual_chat' | 'raid_train')[];
...
export interface StreamingMatchPreferences {
  audienceSizeCompatibility: 'similar' | 'larger_welcome' | 'smaller_welcome' | 'any';
  contentSynergy: 'same_games' | 'complementary' | 'educational' | 'any';
  collaborationFrequency: 'rare' | 'occasional' | 'regular' | 'frequent';
  partnershipGoals: ('audience_growth' | 'content_variety' | 'learning' | 'fun' | 'networking')[];
...
export class AIStreamingMatcher {
   ...
   */
  async findStreamingPartners(criteria: MatchingCriteria): Promise<StreamerMatch[]> {
    const startTime = Date.now();
    logger.info("AI streaming partner matching started", { userId: criteria.userId, criteria });
    ...
    try {
      ...
      if (this.matchingCache.has(cacheKey)) {
        ...
        logger.info("Returned cached streaming matches", { userId: criteria.userId, count: cached.length });
        return cached;
      }
      ...
      const candidates = await this.getStreamingCandidates(criteria, userProfile);
      
      // Apply AI matching algorithms
      const matches = await this.calculateStreamingCompatibility(userProfile, candidates, criteria);
      ...
      logger.info("AI streaming partner matching completed", {
        ...
        averageScore: rankedMatches.reduce((sum, match) => sum + match.compatibilityScore, 0) / rankedMatches.length
      });

      return rankedMatches;

    } catch (error) {
      logger.error("AI streaming partner matching failed", error, { userId: criteria.userId, criteria });
      throw error;
    }
  }
  ...
  async getStreamerProfile(userId: string): Promise<StreamerProfile | null> {
    try {
      ...
      try {
        ...
        matchingPrefs = await storage.getMatchmakingPreferences?.(userId) || {};
      } catch (error) {
        logger.warn("Settings/preferences not available, using defaults", { userId, error });
        userSettings = {};
        matchingPrefs = {};
      }

      // Get real-time platform data
      const platforms = await this.getConnectedPlatforms(userId, user.username || '');
      ...
      const profile: StreamerProfile = {
        ...
        availabilitySchedule: this.extractAvailabilitySchedule(user, userSettings),
        collaborationHistory: await this.getCollaborationHistory(userId),
        matchingPreferences: this.extractMatchingPreferences(matchingPrefs, userSettings)
      };
   ...
   */
  private async getConnectedPlatforms(userId: string, username: string): Promise<ConnectedPlatform[]> {
    ...
    try {
      ...
      logger.error("Failed to get connected platforms", error, { userId, username });
    }
  ...
  private async getStreamingCandidates(criteria: MatchingCriteria, userProfile: StreamerProfile): Promise<StreamerProfile[]> {
    try {
      ...
      logger.error("Failed to get streaming candidates", error, { userId: criteria.userId });
      return [];
    }
  }
  ...
  private async calculateStreamingCompatibility(
    ...
    for (const candidate of candidates) {
      try {
        ...
        const audienceScore = this.calculateAudienceCompatibility(userProfile.audienceMetrics, candidate.audienceMetrics);
        score += audienceScore * 0.25;
        if (audienceScore > 70) reasons.push("Highly compatible audiences");

        // 2. Content Synergy (25% weight)
        const contentScore = this.calculateContentSynergy(userProfile.contentPreferences, candidate.contentPreferences);
        score += contentScore * 0.25;
        if (contentScore > 80) reasons.push("Excellent content alignment");

        // 3. Schedule Alignment (20% weight)
        const scheduleScore = this.calculateScheduleAlignment(userProfile.availabilitySchedule, candidate.availabilitySchedule);
        score += scheduleScore * 0.20;
        if (scheduleScore > 60) reasons.push("Good schedule overlap");

        // 4. Platform Compatibility (15% weight)
        const platformScore = this.calculatePlatformCompatibility(userProfile.platforms, candidate.platforms);
        score += platformScore * 0.15;
        if (platformScore > 50) reasons.push("Shared streaming platforms");

        // 5. Collaboration History (10% weight)
        const historyScore = this.calculateCollaborationScore(candidate.collaborationHistory);
        score += historyScore * 0.10;
        if (historyScore > 80) reasons.push("Excellent collaboration track record");

        // 6. Real-time factors (5% weight)
        const realtimeScore = this.calculateRealtimeFactors(userProfile, candidate);
        ...
        if (score >= 40) {
          const match: StreamerMatch = {
            ...
            suggestedCollaborationType: this.suggestCollaborationType(userProfile, candidate),
            scheduledTimeSlots: this.findCommonTimeSlots(userProfile.availabilitySchedule, candidate.availabilitySchedule),
            audienceOverlapPotential: this.calculateAudienceOverlap(userProfile.audienceMetrics, candidate.audienceMetrics),
            contentSynergyScore: Math.round(contentScore),
            availabilityMatch: Math.round(scheduleScore),
            platformCompatibility: this.getSharedPlatforms(userProfile.platforms, candidate.platforms),
            estimatedViewerBoost: this.estimateViewerBoost(userProfile.audienceMetrics, candidate.audienceMetrics)
          };
  ...
  private calculateAudienceCompatibility(user: AudienceMetrics, candidate: AudienceMetrics): number {
    let score = 0;

    // Audience size compatibility
    const sizeRatio = Math.min(user.totalFollowers, candidate.totalFollowers) / Math.max(user.totalFollowers, candidate.totalFollowers);
    score += sizeRatio * 30;

    // Engagement compatibility
    const engagementDiff = Math.abs(user.engagementRate - candidate.engagementRate);
    ...
    // Regional overlap
    const sharedRegions = user.audienceRegions.filter(region => candidate.audienceRegions.includes(region));
    score += (sharedRegions.length / Math.max(user.audienceRegions.length, candidate.audienceRegions.length)) * 20;

    return Math.min(score, 100);
  }

  private calculateContentSynergy(user: ContentPreferences, candidate: ContentPreferences): number {
    let score = 0;

    // Shared primary games
    const sharedPrimary = user.primaryGames.filter(game => candidate.primaryGames.includes(game));
    score += (sharedPrimary.length / Math.max(user.primaryGames.length, 1)) * 40;

    // Complementary games
    const sharedSecondary = user.secondaryGames.filter(game => candidate.secondaryGames.includes(game));
    score += (sharedSecondary.length / Math.max(user.secondaryGames.length, 1)) * 20;

    // Content type alignment
    const sharedTypes = user.contentTypes.filter(type => candidate.contentTypes.includes(type));
    score += (sharedTypes.length / Math.max(user.contentTypes.length, 1)) * 25;

    // Collaboration type compatibility
    const sharedCollabTypes = user.collabTypes.filter(type => candidate.collabTypes.includes(type));
    score += (sharedCollabTypes.length / Math.max(user.collabTypes.length, 1)) * 15;

    return Math.min(score, 100);
  }

  private calculateScheduleAlignment(user: AvailabilitySchedule, candidate: AvailabilitySchedule): number {
    let score = 0;
    let totalSlots = 0;
    let matchingSlots = 0;
    ...
    Object.keys(user.weeklySchedule).forEach(day => {
      const userDay = user.weeklySchedule[day as keyof typeof user.weeklySchedule];
      const candidateDay = candidate.weeklySchedule[day as keyof typeof candidate.weeklySchedule];
    ...
    const timezoneScore = this.calculateTimezoneCompatibility(user.timezone, candidate.timezone);
    score = (score * 0.7) + (timezoneScore * 0.3);

    return Math.min(score, 100);
  }

  private calculatePlatformCompatibility(userPlatforms: ConnectedPlatform[], candidatePlatforms: ConnectedPlatform[]): number {
    const userPlatformNames = userPlatforms.filter(p => p.isActive).map(p => p.platform);
    const candidatePlatformNames = candidatePlatforms.filter(p => p.isActive).map(p => p.platform);
    
    const sharedPlatforms = userPlatformNames.filter(platform => candidatePlatformNames.includes(platform));
    const totalUniquePlatforms = new Set([...userPlatformNames, ...candidatePlatformNames]).size;
  ...
  private calculateCollaborationScore(history: CollaborationHistory): number {
    ...
    if (history.totalCollaborations > 0) {
      const successRate = (history.successfulCollaborations / history.totalCollaborations) * 100;
    ...
    if (history.lastCollaboration && Date.now() - history.lastCollaboration.getTime() < 30 * 24 * 60 * 60 * 1000) {
      score += 10;
    }
  ...
  private calculateRealtimeFactors(user: StreamerProfile, candidate: StreamerProfile): number {
    ...
    const hasRecentActivity = candidate.platforms.some(p => 
      p.lastStreamDate && Date.now() - p.lastStreamDate.getTime() < recentThreshold
    );
  ...
  private suggestCollaborationType(user: StreamerProfile, candidate: StreamerProfile): string {
  ...
  private findCommonTimeSlots(user: AvailabilitySchedule, candidate: AvailabilitySchedule): string[] {
    ...
    Object.keys(user.weeklySchedule).forEach(day => {
      ...
      if (userDay.available && candidateDay.available) {
        userDay.timeSlots.forEach(userSlot => {
          candidateDay.timeSlots.forEach(candidateSlot => {
            if (this.timeSlotsOverlap(userSlot, candidateSlot)) {
              commonSlots.push(`${day}: ${this.calculateOverlapTime(userSlot, candidateSlot)}`);
            }
          });
        });
      }
  ...
  private calculateAudienceOverlap(user: AudienceMetrics, candidate: AudienceMetrics): number {
    const sharedRegions = user.audienceRegions.filter(region => candidate.audienceRegions.includes(region));
    const totalRegions = new Set([...user.audienceRegions, ...candidate.audienceRegions]).size;
    
    if (totalRegions === 0) return 0;
    return Math.round((sharedRegions.length / totalRegions) * 100);
  }

  private getSharedPlatforms(user: ConnectedPlatform[], candidate: ConnectedPlatform[]): string[] {
    const userPlatforms = user.filter(p => p.isActive).map(p => p.platform);
    const candidatePlatforms = candidate.filter(p => p.isActive).map(p => p.platform);
    
    return userPlatforms.filter(platform => candidatePlatforms.includes(platform));
  }

  private estimateViewerBoost(user: AudienceMetrics, candidate: AudienceMetrics): number {
    // Conservative estimate: 10-30% of smaller audience might cross over
    const smallerAudience = Math.min(user.averageViewers, candidate.averageViewers);
  ...
  private async calculateAudienceMetrics(platforms: ConnectedPlatform[]): Promise<AudienceMetrics> {
    const totalFollowers = platforms.reduce((sum, p) => sum + (p.followerCount || 0), 0);
    const avgViewers = platforms.reduce((sum, p) => sum + (p.averageViewers || 0), 0);
    ...
    return {
      ...
      chatActivity: avgViewers > 100 ? 'high' : avgViewers > 50 ? 'medium' : 'low'
    };
  }
  ...
  private extractAvailabilitySchedule(user: any, userSettings: any): AvailabilitySchedule {
    ...
    return {
      ...
      weeklySchedule: {
        ...
        saturday: { available: true, timeSlots: ["14:00-17:00", "19:00-22:00"] },
  ...
  private async getCollaborationHistory(userId: string): Promise<CollaborationHistory> {
    try {
  ...
  private extractMatchingPreferences(matchingPrefs: any, userSettings: any): StreamingMatchPreferences {
    return {
   ...
   */
  async getScheduledMatches(userId: string, timeSlot?: string): Promise<StreamerMatch[]> {
    return this.findStreamingPartners({
      userId,
