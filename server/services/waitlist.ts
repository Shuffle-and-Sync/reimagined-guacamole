...
export class WaitlistService {
   ...
   */
  async joinEventWithWaitlist(eventId: string, userId: string, requestedPlayerType: 'main' | 'alternate' = 'main') {
    try {
      ...
      const alternates = attendees.filter(a => a.playerType === 'alternate' && a.status === 'attending').length;
      ...
      if (requestedPlayerType === 'main') {
        if (mainPlayers < playerSlots) {
          ...
          throw new Error('Event is full. Please try again later or contact the organizer.');
        }
      } else {
      ...
      if (assignedPlayerType === 'alternate') {
        logger.info('User added to waitlist', { eventId, userId, position: waitlistPosition });
      }

      return {
        attendee,
        playerType: assignedPlayerType,
        waitlistPosition: assignedPlayerType === 'alternate' ? waitlistPosition : 0,
        isWaitlisted: assignedPlayerType === 'alternate',
      };
    } catch (error) {
      logger.error('Failed to join event with waitlist', error, { eventId, userId });
      throw error;
    }
  }
  ...
  async promoteFromWaitlist(eventId: string) {
    try {
      ...
      const attendees = await storage.getEventAttendees(eventId);
      const mainPlayers = attendees.filter(a => a.playerType === 'main' && a.status === 'attending');
      const alternates = attendees.filter(a => a.playerType === 'alternate' && a.status === 'attending')
        .sort((a, b) => {
      ...
      if (mainPlayers.length < playerSlots && alternates.length > 0) {
        ...
        await enhancedNotificationService.sendWaitlistPromotedNotification(eventId, toPromote.userId);
  ...
  async getWaitlistPosition(eventId: string, userId: string): Promise<number> {
    try {
      ...
      logger.error('Failed to get waitlist position', error, { eventId, userId });
      return 0;
    }
  }
