...
export interface CodeIssue {
  ...
  category: 'typescript' | 'express' | 'drizzle' | 'security' | 'performance' | 'style';
  message: string;
  rule?: string;
  suggestion?: string;
  autoFixable?: boolean;
...
export class BackendCopilotAgent {
  ...
  async analyzeBackend(): Promise<AnalysisResult> {
    ...
    try {
      ...
      const drizzleAnalysis = await analyzeDrizzleUsage(this.config.projectRoot);
      ...
      if (drizzleAnalysis.optimizationSuggestions.length > 0) {
        logger.info(`📊 Found ${drizzleAnalysis.totalQueries} database queries with ${drizzleAnalysis.optimizationSuggestions.length} optimization opportunities`);
      }
      ...
      const summary = this.generateSummary(totalFiles, issues, issuesByCategory);
      ...
      logger.info(`✅ Analysis complete: ${issues.length} issues found in ${totalFiles} files`);
      return result;
      ...
      lines.forEach((line, index) => {
        ...
        if (line.includes('async (req, res') && !content.includes('try {')) {
          issues.push({
            ...
            suggestion: 'Wrap async route handlers with try-catch or use asyncHandler middleware',
            autoFixable: false
          });
        }

        // 2. Missing input validation
        if (line.includes('req.body') && !content.includes('zod') && !content.includes('validate')) {
          issues.push({
            file: path.relative(this.config.projectRoot, file),
            line: index + 1,
            severity: 'warning',
   ...
   */
  private generateSummary(totalFiles: number, issues: CodeIssue[], categories: Record<string, number>): string {
  ...
  private generateRecommendations(issues: CodeIssue[]): string[] {
    ...
    if (recommendations.length === 0) {
      recommendations.push('✅ Backend code follows best practices - great work!');
    }
 ...
 */
export async function runBackendAnalysis(config?: Partial<CopilotAgentConfig>): Promise<AnalysisResult> {
 ...
 */
export async function generateBackendReport(outputPath: string, config?: Partial<CopilotAgentConfig>): Promise<void> {
  const agent = new BackendCopilotAgent(config);
