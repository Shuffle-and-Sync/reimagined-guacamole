...
export class DrizzleAnalyzer {
    ...
    try {
      ...
      for (const file of files) {
        ...
        lines.forEach((line, index) => {
          this.analyzeQueryPerformance(file, line, index + 1, content, issues, performanceMetrics);
          this.analyzeTransactionUsage(file, line, index + 1, content, issues, performanceMetrics);
          this.analyzeRelationshipPatterns(file, line, index + 1, content, issues, performanceMetrics);
          this.analyzeTypeUsage(file, line, index + 1, content, issues);
        });
      }

      // Generate optimization suggestions
      optimizationSuggestions.push(...this.generateOptimizationSuggestions(performanceMetrics));

      logger.info(`✅ Drizzle analysis complete: ${totalQueries} queries analyzed, ${issues.length} issues found`);
  ...
  private analyzeQueryPerformance(
    ...
    if (line.includes('db.select') && !line.includes('where') && !content.includes('.where(')) {
      issues.push({
        file: relativePath,
        line: lineNumber,
        severity: 'warning',
        ...
        suggestion: 'Add WHERE clause to limit query results and improve performance',
        autoFixable: false
      });
    }
    ...
    if ((line.includes('db.select') || line.includes('.findMany')) && 
      ...
      issues.push({
        ...
        rule: 'add-pagination',
        suggestion: 'Add limit/offset or cursor-based pagination to prevent large result sets',
        autoFixable: false
      });
    }
    ...
    if (line.includes('for (') || line.includes('forEach(')) {
      const nextFewLines = content.split('\n').slice(lineNumber, lineNumber + 5).join('\n');
      if (nextFewLines.includes('db.select') || nextFewLines.includes('db.query')) {
        ...
        issues.push({
          ...
          rule: 'fix-n-plus-one',
          suggestion: 'Use joins or batch queries instead of queries inside loops',
          autoFixable: false
        });
      }
    }
  ...
  private analyzeTransactionUsage(
    ...
    if (hasMultipleOperations && !content.includes('db.transaction') && 
        (line.includes('db.insert') || line.includes('db.update') || line.includes('db.delete'))) {
      ...
      issues.push({
        ...
        suggestion: 'Wrap multiple related database operations in a transaction for data consistency',
        autoFixable: false
      });
    }
  }
  ...
  private analyzeRelationshipPatterns(
    ...
    if (line.includes('db.select') && content.includes('relations') && !content.includes('with:')) {
      issues.push({
        file: relativePath,
        line: lineNumber,
        severity: 'info',
        ...
        suggestion: 'Use "with" clause to eagerly load related data and avoid N+1 queries',
        autoFixable: false
      });
    }

    // Inefficient joins
    if (line.includes('.leftJoin') || line.includes('.rightJoin') || line.includes('.innerJoin')) {
  ...
  private analyzeTypeUsage(
    ...
    if (line.includes('db.select') && !line.includes(': ') && line.includes('=')) {
      issues.push({
        file: relativePath,
        line: lineNumber,
        severity: 'info',
        ...
        suggestion: 'Add explicit type annotation for query results to improve type safety',
        autoFixable: false
      });
    }

    // Use of any type in query operations
    if ((line.includes('db.') || line.includes('query.')) && line.includes(': any')) {
      issues.push({
        file: relativePath,
        line: lineNumber,
        severity: 'warning',
   ...
   */
  private generateOptimizationSuggestions(metrics: DrizzleAnalysisResult['performanceMetrics']): string[] {
    const suggestions: string[] = [];

    if (metrics.queriesWithoutIndexes > 5) {
      suggestions.push('🔍 Consider adding database indexes for frequently queried columns to improve performance');
    }

    if (metrics.queriesWithoutTransactions > 0) {
      suggestions.push('🔄 Implement transactions for multi-operation database workflows to ensure data consistency');
    }

    if (metrics.potentialN1Problems > 0) {
      suggestions.push('⚡ Optimize queries to prevent N+1 problems by using joins or batch operations');
    }

    if (metrics.missingPagination > 3) {
      suggestions.push('📄 Implement pagination for list endpoints to prevent performance issues with large datasets');
    }
 ...
 */
export async function analyzeDrizzleUsage(projectRoot = process.cwd()): Promise<DrizzleAnalysisResult> {
