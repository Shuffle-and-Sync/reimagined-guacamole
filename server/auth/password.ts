...
export function validatePasswordStrength(password: string): {
  ...
  const hasAscendingSequence =
    /(?:012|123|234|345|456|567|678|789|890|abc|bcd|cde|def|efg|fgh|ghi|hij|ijk|jkl|klm|lmn|mno|nop|opq|pqr|qrs|rst|stu|tuv|uvw|vwx|wxy|xyz)/.test(
      lowercasePassword,
    );
  const hasDescendingSequence =
    /(?:987|876|765|654|543|432|321|210|109|zyx|yxw|xwv|wvu|vut|uts|tsr|srq|rqp|qpo|pon|onm|nml|mlk|lkj|jih|ihg|hgf|gfe|fed|edc|dcb|cba)/.test(
      lowercasePassword,
    );
...
const authFailures = new Map<
  string,
  { failures: number; lastFailure: number; lockUntil?: number }
>();
...
export function checkAuthRateLimit(
  ...
  if (Math.random() < 0.01) {
    // 1% chance to clean up on each check
    const keysToDelete: string[] = [];
    authFailures.forEach((value, key) => {
      if (now - value.lastFailure > 24 * 60 * 60 * 1000) {
        // 24 hours
  ...
  if (attempts.lockUntil && now < attempts.lockUntil) {
    return {
      allowed: false,
      retryAfter: Math.ceil((attempts.lockUntil - now) / 1000),
    };
  }
  ...
  const lockDuration = Math.min(
    15 * 60 * 1000 * Math.pow(2, attempts.failures - 5),
    24 * 60 * 60 * 1000,
  ); // Max 24 hours
