...
const JWT_SECRET = new TextEncoder().encode(
  process.env.AUTH_SECRET || 'your-secure-secret-key-at-least-32-characters-long'
);
...
export async function generateEmailVerificationJWT(
  ...
  try {
    ...
    logger.error('Failed to generate email verification token', { userId, error });
...
export async function verifyEmailVerificationJWT(token: string): Promise<{
  ...
  try {
    ...
    return {
      valid: true,
      payload: payload as unknown as TokenPayload & { type: string; email: string }
    };
  } catch (error) {
...
export async function generateRefreshTokenJWT(
  ...
  if (!userId || !email || !tokenId) {
    throw new Error('User ID, email, and token ID are required for refresh token');
  }
...
export async function verifyRefreshTokenJWT(token: string): Promise<{
  ...
  // Basic token security validation
  if (!isSecureToken(token.replace(/^Bearer\s+/i, ''))) {
    logger.warn('Potentially insecure token detected in refresh token verification');
  }
...
export async function verifyPasswordResetJWT(token: string): Promise<{
  ...
  // Basic token security validation
  if (!isSecureToken(token.replace(/^Bearer\s+/i, ''))) {
    logger.warn('Potentially insecure token detected in password reset verification');
  }
 ...
 */
export async function verifyAccessTokenJWT(token: string, options?: {
