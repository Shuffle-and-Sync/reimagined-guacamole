...
router.delete('/clear', async (req, res) => {
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'production') {
      return res.status(403).json({ success: false, error: 'Cache clearing disabled in production' });
    }
    ...
    if (confirmHeader !== 'yes-delete-all-cache') {
      return res.status(400).json({ 
        success: false, 
        error: 'Add header X-Confirm-Cache-Clear: yes-delete-all-cache to confirm'
      });
    }
