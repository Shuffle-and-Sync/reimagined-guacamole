  ...
  try {
    ...
    res.json({
      ...
      summary: {
        ...
        healthy: Array.from(healthChecks.values()).filter(h => h.status === 'healthy').length,
        degraded: Array.from(healthChecks.values()).filter(h => h.status === 'degraded').length,
        unhealthy: Array.from(healthChecks.values()).filter(h => h.status === 'unhealthy').length,
      },
      timestamp: new Date().toISOString(),
    });
  } catch (error) {
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      if (confirmHeader !== 'yes-stop-monitoring') {
        return res.status(400).json({
          success: false,
          error: 'Add header X-Confirm-Stop-Monitoring: yes-stop-monitoring to confirm'
        });
      }
    }
...
router.post('/alerts/test', async (req, res) => {
  ...
  try {
    ...
    const { severity = 'info', message = 'Test alert from monitoring system' } = req.body;
