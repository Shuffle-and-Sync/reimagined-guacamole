...
import { requireAuth as isAuthenticated, type AuthenticatedRequest } from '../auth/auth.middleware';
 ...
 */
router.post('/announcement', isAuthenticated, async (req: Request, res: Response) => {
  const authenticatedReq = req as AuthenticatedRequest;
  
  try {
    const { title, message, userIds, priority, actionUrl, actionText, communityId } = req.body;
    
    if (!title || !message || !userIds || !Array.isArray(userIds)) {
      return res.status(400).json({ message: 'Missing required fields: title, message, userIds' });
    }
 ...
 */
router.post('/digest/weekly', isAuthenticated, async (req: Request, res: Response) => {
  const authenticatedReq = req as AuthenticatedRequest;
 ...
 */
router.post('/websocket/register', isAuthenticated, async (req: Request, res: Response) => {
  const authenticatedReq = req as AuthenticatedRequest;
