    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      if (confirmHeader !== 'yes-run-infrastructure-tests') {
        return res.status(400).json({
          success: false,
          error: 'Add header X-Confirm-Run-Tests: yes-run-infrastructure-tests to confirm'
        });
      }
    }
...
router.post('/suite/:suiteName', async (req, res) => {
  ...
  try {
    ...
    logger.info('Specific test suite initiated by admin', { userId, suiteName });
      ...
      availableTestSuites: [
        {
          ...
          tests: ['service_status', 'metrics_collection', 'health_checks', 'alert_system']
        },
        {
          name: 'Caching Layer',
          description: 'Test Redis caching, fallback mechanisms, and cache operations',
          tests: ['service_status', 'cache_set', 'cache_get', 'cache_delete', 'graceful_degradation']
        },
        {
          name: 'Database Optimization',
          description: 'Test database connection pooling, query optimization, and performance monitoring',
          tests: ['connectivity', 'optimized_queries', 'connection_pool', 'performance_monitoring']
        },
        {
          name: 'Backup System',
          description: 'Test backup service, backup verification, and recovery capabilities',
          tests: ['service_status', 'configuration', 'backup_directory', 'cleanup_system']
        },
        {
          name: 'Analytics System',
          description: 'Test analytics data collection, metrics tracking, and reporting',
          tests: ['data_models', 'storage_operations', 'metrics_collection', 'api_endpoints']
        },
        {
          name: 'Notification System',
          description: 'Test notification delivery, preferences, and multi-channel support',
          tests: ['data_models', 'preferences_system', 'delivery_channels', 'queue_system']
        },
        {
          name: 'Health Check System',
          description: 'Test comprehensive health monitoring and status reporting',
          tests: ['database_health', 'redis_health', 'application_health', 'filesystem_health', 'overall_status']
        },
        {
          name: 'Integration Scenarios',
          description: 'Test cross-system integration and end-to-end workflows',
          tests: ['monitoring_alerting', 'cache_database', 'monitoring_database', 'backup_monitoring', 'system_resilience']
        }
      ],
      instructions: {
