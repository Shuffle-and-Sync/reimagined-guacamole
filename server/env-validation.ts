...
const REQUIRED_DEVELOPMENT_VARS = ["DATABASE_URL", "AUTH_SECRET"] as const;
...
const VALIDATION_RULES = {
  STREAM_KEY_ENCRYPTION_KEY: (value: string) => {
    if (value.length !== 32) {
      throw new Error(
        "STREAM_KEY_ENCRYPTION_KEY must be exactly 32 characters long. Generate with: openssl rand -hex 16",
      );
    }
    return true;
  },
  DATABASE_URL: (value: string) => {
    ...
    if (
      ...
      throw new Error(
        "DATABASE_URL must be a valid SQLite Cloud connection string (sqlitecloud://) or local SQLite file path",
      );
    }
    return true;
  },
  AUTH_URL: (value: string) => {
    try {
      ...
      throw new Error(
        "AUTH_URL must be a valid URL (e.g., https://your-domain.com)",
      );
    }
  },
  ...
  GOOGLE_CLIENT_ID: (value: string) => {
    if (!value || value.includes("demo-") || value.includes("your-")) {
      throw new Error(
        "GOOGLE_CLIENT_ID must be a valid Google OAuth client ID from Google Console",
      );
    }
    return true;
  },
  GOOGLE_CLIENT_SECRET: (value: string) => {
    if (!value || value.includes("demo-") || value.includes("your-")) {
      throw new Error(
        "GOOGLE_CLIENT_SECRET must be a valid Google OAuth client secret from Google Console",
      );
    }
    return true;
  },
  SENDGRID_API_KEY: (value: string) => {
    if (value && (!value.startsWith("SG.") || value.includes("demo-"))) {
      throw new Error(
        'SENDGRID_API_KEY must be a valid SendGrid API key starting with "SG."',
      );
    }
    return true;
  },
  TWITCH_CLIENT_ID: (value: string) => {
    if (
      value &&
      (value.includes("demo-") || value.includes("your-") || value.length < 10)
    ) {
      throw new Error(
        "TWITCH_CLIENT_ID must be a valid Twitch client ID from Twitch Developer Console",
      );
    }
    return true;
  },
  TWITCH_CLIENT_SECRET: (value: string) => {
    if (
      ...
      throw new Error(
        "TWITCH_CLIENT_SECRET must be a valid Twitch client secret from Twitch Developer Console",
      );
    }
    return true;
  },
  YOUTUBE_API_KEY: (value: string) => {
    if (
      ...
      throw new Error(
        "YOUTUBE_API_KEY must be a valid YouTube API key from Google Cloud Console",
      );
    }
    return true;
  },
  DISCORD_BOT_TOKEN: (value: string) => {
    if (
      ...
      throw new Error(
        "DISCORD_BOT_TOKEN must be a valid Discord bot token from Discord Developer Portal",
      );
    }
    return true;
  },
  ...
  SENTRY_DSN: (value: string) => {
    if (
      ...
      throw new Error(
        'SENTRY_DSN must be a valid Sentry DSN URL starting with "https://" or "http://"',
      );
    }
    return true;
  },
  ...
  DATABASE_DIRECT_URL: (value: string) => {
    ...
    if (
      ...
      throw new Error(
        "DATABASE_DIRECT_URL must be a valid SQLite Cloud connection string (sqlitecloud://) or local SQLite file path",
      );
    }
    return true;
  },
  AUTH_TRUST_HOST: (value: string) => {
    ...
    if (!["true", "false", "1", "0"].includes(lowerValue)) {
      throw new Error(
        "AUTH_TRUST_HOST must be a boolean value (true, false, 1, or 0)",
      );
    }
    return true;
  },
  ...
  ALLOWED_ORIGINS: (value: string) => {
    ...
    for (const origin of origins) {
      ...
      try {
        ...
        throw new Error(
          `ALLOWED_ORIGINS contains invalid URL: ${origin}. Must be comma-separated valid URLs or "*"`,
        );
      }
    }
    return true;
  ...
  FACEBOOK_APP_ID: (value: string) => {
    if (
      ...
      throw new Error(
        "FACEBOOK_APP_ID must be a valid Facebook app ID from Facebook Developer Console",
      );
    }
    return true;
  },
  FACEBOOK_APP_SECRET: (value: string) => {
    if (
      ...
      throw new Error(
        "FACEBOOK_APP_SECRET must be a valid Facebook app secret from Facebook Developer Console",
      );
    }
    return true;
  },
  FACEBOOK_WEBHOOK_VERIFY_TOKEN: (value: string) => {
    if (
      value &&
      (value.includes("demo-") || value.includes("your-") || value.length < 16)
    ) {
      throw new Error(
        "FACEBOOK_WEBHOOK_VERIFY_TOKEN must be a secure random token (16+ characters)",
      );
    }
    return true;
  },
  YOUTUBE_CLIENT_ID: (value: string) => {
    if (
      ...
      throw new Error(
        "YOUTUBE_CLIENT_ID must be a valid YouTube OAuth client ID from Google Cloud Console",
      );
    }
    return true;
  },
  YOUTUBE_CLIENT_SECRET: (value: string) => {
    if (
      ...
      throw new Error(
        "YOUTUBE_CLIENT_SECRET must be a valid YouTube OAuth client secret from Google Cloud Console",
      );
    }
    return true;
  },
  YOUTUBE_WEBHOOK_VERIFY_TOKEN: (value: string) => {
    if (
      ...
      throw new Error(
        "YOUTUBE_WEBHOOK_VERIFY_TOKEN must be a secure random token (16+ characters)",
      );
    }
    return true;
  },
  TWITCH_EVENTSUB_SECRET: (value: string) => {
    if (
      ...
      throw new Error(
        "TWITCH_EVENTSUB_SECRET must be a secure random token (16+ characters). Generate with: openssl rand -hex 16",
      );
    }
    return true;
  },
  SENDGRID_SENDER: (value: string) => {
    ...
    if (value && !emailRegex.test(value)) {
      throw new Error(
        "SENDGRID_SENDER must be a valid email address (e.g., noreply@yourdomain.com)",
      );
    }
    return true;
  },
    ...
    for (const varName of sensitiveVars) {
      ...
      if (value) {
        ...
        if (
          ...
          issues.push(
            `${varName} appears to contain demo/test values in production`,
          );
        }
      }
    }
...
export function validateEnvironmentVariables(): EnvValidationResult {
  ...
  if (process.env.NODE_ENV === "production" && !process.env.PORT) {
    warnings.push(
      "PORT environment variable should be set by the deployment platform in production",
    );
  }
...
export function validateAndLogEnvironment(): void {
  ...
  if (!result.isValid) {
    ...
    if (process.env.NODE_ENV === "production") {
      logger.warn(
        "Starting server with degraded functionality due to missing environment variables",
        {
          missingRequired: result.missingRequired,
        },
      );
    ...
    logger.info("Environment validation passed", {
      ...
      requiredVarsSet:
        getRequiredVariables().length - result.missingRequired.length,
      recommendedVarsSet:
        RECOMMENDED_VARS.length - result.missingRecommended.length,
  ...
  if (result.missingRequired.length > 0) {
    ...
    for (const varName of result.missingRequired) {
      switch (varName) {
        case "DATABASE_URL":
          messages.push(
            "  • DATABASE_URL: Set up SQLite Cloud or local SQLite database",
          );
          messages.push(
            "    Example (Cloud): sqlitecloud://host:port/db?apikey=YOUR_API_KEY",
          );
          messages.push("    Example (Local): ./dev.db");
          break;
        case "AUTH_SECRET":
          messages.push(
            "  • AUTH_SECRET: Generate secure secret (32+ characters)",
          );
        ...
        default:
          messages.push(
            `  • ${varName}: Check .env.example for setup instructions`,
          );
      }
    }
    messages.push("");
