...
router.get('/:game_id/cards/search', async (req, res) => {
  try {
    ...
    logger.info('Game-scoped card search', { gameId: game_id, query: params.q });
    ...
    logger.error('Error in game-scoped card search', error, { gameId: req.params.game_id });
...
router.get('/:game_id/cards/:id', async (req, res) => {
  try {
    ...
    logger.error('Error getting card by ID', error, { gameId: req.params.game_id, cardId: req.params.id });
...
router.get('/:game_id/cards/named', async (req, res) => {
  try {
    ...
    logger.info('Get card by name in game', { gameId: game_id, name: cardName });
    ...
    logger.error('Error getting card by name', error, { gameId: req.params.game_id });
    return res.status(500).json({ message: 'Failed to fetch card' });
  }
});
...
router.get('/:game_id/cards/autocomplete', async (req, res) => {
  try {
    ...
    const result = await universalCardService.autocomplete(game_id, params.q, params.limit);
    ...
    logger.error('Error in autocomplete', error, { gameId: req.params.game_id });
...
router.get('/:game_id/cards/random', async (req, res) => {
  try {
    ...
    logger.error('Error getting random card', error, { gameId: req.params.game_id });
