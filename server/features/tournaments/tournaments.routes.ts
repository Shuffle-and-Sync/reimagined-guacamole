...
router.delete("/:id/leave", isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    const success = await tournamentsService.leaveTournament(
      tournamentId,
      userId,
    );
    if (success) {
...
router.post("/:id/advance", isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    const tournament = await tournamentsService.advanceRound(
      tournamentId,
      userId,
    );
    res.json(tournament);
  } catch (error) {
    logger.error("Failed to advance tournament round", error, {
      userId: getAuthUserId(authenticatedReq),
      tournamentId: req.params.id,
    });
    res.status(500).json({
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const result = await tournamentsService.reportMatchResult(
        tournamentId,
        matchId,
        winnerId,
        userId,
...
router.get("/:id/details", async (req, res) => {
  try {
    ...
    logger.error("Failed to fetch tournament details", error, {
      tournamentId: req.params.id,
    });
...
router.post(
  "/:id/matches/:matchId/create-session",
  isAuthenticated,
  async (req, res) => {
    const authenticatedReq = req as AuthenticatedRequest;
    try {
      ...
      const session = await tournamentsService.createMatchGameSession(
        tournamentId,
        matchId,
        userId,
      );
...
router.patch("/:id", isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    const updatedTournament = await tournamentsService.updateTournament(
      tournamentId,
      updates,
      userId,
    );
    ...
    if (error instanceof Error) {
      ...
      if (
        error.message.includes("Only the tournament organizer") ||
