...
export class ExternalServiceError extends AppError {
  constructor(service: string, message: string = 'External service unavailable', context?: Record<string, any>) {
...
export function handleDatabaseError(error: any): never {
  ...
  if (error.code === 'ECONNREFUSED') {
    throw new DatabaseError('Database connection refused', { code: error.code });
  } else if (error.code === 'ENOTFOUND') {
    throw new DatabaseError('Database host not found', { code: error.code });
  } else if (error.code === '28P01') {
    throw new DatabaseError('Database authentication failed', { code: error.code });
 ...
 */
export function validateAndThrow(condition: boolean, message: string, ErrorClass = ValidationError): void {
  if (!condition) {
