...
describe('Enhanced Input Sanitization Security Tests', () => {
  ...
  describe('Enhanced SQL Injection Protection', () => {
    ...
    test('should log multiple detected patterns with enhanced context', () => {
      const complexInput = "'; DROP TABLE users; SELECT * FROM information_schema.tables; EXEC sp_adduser --";
    ...
    test('should handle nested objects with complex attacks', () => {
      const nestedAttack = {
        ...
        metadata: [
          "0x41424344",
          "CONVERT(varchar, sensitive_data)"
        ]
      };
      ...
      expect(sanitized.search.filters.category).not.toContain('information_schema');
