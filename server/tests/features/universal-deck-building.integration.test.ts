...
describe('Game Creator API - Integration Tests', () => {
  ...
  describe('Game Service Integration', () => {
    ...
    it('should update game', async () => {
      ...
      const result = await gameService.updateGame('game-123', 'user-123', { displayName: 'Updated Name' });
  ...
  describe('Universal Card Service Integration', () => {
    it('should search cards for a specific game', async () => {
      ...
      (universalCardService.searchCards as jest.Mock).mockResolvedValue(mockResult);

      const result = await universalCardService.searchCards('custom-game', 'dragon', { page: 1, limit: 20 });
    ...
    it('should get card by ID for a specific game', async () => {
      ...
      (universalCardService.getCardById as jest.Mock).mockResolvedValue(mockCard);

      const result = await universalCardService.getCardById('custom-game', 'card-123');
    ...
    it('should get card by name', async () => {
      ...
      (universalCardService.getCardByName as jest.Mock).mockResolvedValue(mockCard);

      const result = await universalCardService.getCardByName('custom-game', 'Lightning Bolt', {});
    ...
    it('should autocomplete card names', async () => {
      ...
      (universalCardService.autocomplete as jest.Mock).mockResolvedValue(mockResult);

      const result = await universalCardService.autocomplete('custom-game', 'light', 10);
    ...
    it('should get random card', async () => {
      ...
      (universalCardService.getRandomCard as jest.Mock).mockResolvedValue(mockCard);

      const result = await universalCardService.getRandomCard('custom-game', {});
  ...
  describe('Adapter Selection Logic', () => {
    it('should use Scryfall adapter for mtg-official game', async () => {
      ...
      const result = await universalCardService.searchCards('mtg-official', 'lotus', {});
    ...
    it('should use Custom adapter for user-defined games', async () => {
      ...
      const result = await universalCardService.searchCards('my-game', 'card', {});
