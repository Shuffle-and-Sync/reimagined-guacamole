...
import { cardRecognitionService, type MtgCard } from '../../services/card-recognition';
...
describe('Card Recognition Service', () => {
  ...
  describe('searchCards', () => {
    ...
    test('should return empty results for non-existent card', async () => {
      const result = await cardRecognitionService.searchCards('NonExistentCardXYZ123');
  ...
  describe('getCardById', () => {
    test('should get card by Scryfall ID', async () => {
      ...
      const searchResult = await cardRecognitionService.searchCards('Lightning Bolt', { limit: 1 });
  ...
  describe('getCardByName', () => {
    ...
    test('should return null for non-existent card name', async () => {
      const card = await cardRecognitionService.getCardByName('This Card Does Not Exist XYZ');
      expect(card).toBeNull();
    });
  ...
  describe('getRandomCard', () => {
    ...
    test('should filter by format', async () => {
      const card = await cardRecognitionService.getRandomCard({ format: 'standard' });
