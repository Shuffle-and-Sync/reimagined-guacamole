...
describe("ScryfallAdapter", () => {
  ...
  describe("searchCards", () => {
    it("should search cards and transform to universal format", async () => {
      ...
      expect(cardRecognitionService.searchCards).toHaveBeenCalledWith(
        "bolt",
        undefined,
      );
    });
  ...
  describe("getCardByName", () => {
    it("should get card by name", async () => {
      ...
      const result = await scryfallAdapter.getCardByName("Mox Ruby", {
        set: "lea",
      });

      expect(result).toBeDefined();
      expect(result?.name).toBe("Mox Ruby");
      expect(cardRecognitionService.getCardByName).toHaveBeenCalledWith(
        "Mox Ruby",
        { set: "lea" },
      );
    });
  ...
  describe("autocomplete", () => {
    it("should autocomplete card names", async () => {
      const mockResult = {
        suggestions: [{ name: "Lightning Bolt" }, { name: "Lightning Strike" }],
      };

      (cardRecognitionService.autocomplete as jest.Mock).mockResolvedValue(
        mockResult,
      );
      ...
      expect(cardRecognitionService.autocomplete).toHaveBeenCalledWith(
        "light",
        10,
      );
    });
  ...
  describe("getRandomCard", () => {
    it("should get random card", async () => {
      ...
      const result = await scryfallAdapter.getRandomCard({
        format: "commander",
      });

      expect(result).toBeDefined();
      expect(result.gameId).toBe("mtg-official");
      expect(cardRecognitionService.getRandomCard).toHaveBeenCalledWith({
        format: "commander",
      });
    });
  });
...
describe.skip("CustomGameAdapter", () => {
  ...
  describe("searchCards", () => {
    it("should search cards from database", async () => {
      ...
      const result = await adapter.searchCards("dragon", {
        limit: 20,
        page: 1,
      });
