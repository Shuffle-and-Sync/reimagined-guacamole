...
export function CoordinationDashboard({
  eventId,
  isHost = false,
}: CoordinationDashboardProps) {
  const [selectedPhase, setSelectedPhase] = useState<string>("");
  const { user } = useAuth();
  const { toast } = useToast();
  const { data: coordinationStatus, isLoading } =
    useCoordinationStatus(eventId);
  ...
  useEffect(() => {
    ...
    const unsubscribePhaseUpdated = collaborativeStreamingWS.onPhaseUpdated(
      (data) => {
    ...
    const unsubscribePhaseError = collaborativeStreamingWS.onPhaseChangeError(
      (data) => {
        if (data.eventId === eventId) {
          toast({
  ...
  const currentPhase =
    COORDINATION_PHASES.find((p) => p.name === status?.currentPhase) ??
    COORDINATION_PHASES[0];
        ...
        <CardContent className="space-y-4">
          {!status?.actualStartTime ? (
            <Alert>
              ...
              <AlertDescription>
                Coordination session not started. Start the session to begin
            ...
            <div className="flex items-center justify-between">
              ...
              {status?.actualStartTime && (
                <span className="text-sm text-muted-foreground">
                  Duration: {Math.floor(mockMetrics.streamDuration / 60)}h{" "}
                  {mockMetrics.streamDuration % 60}m
                </span>
              )}
            </div>
          )}
              ...
              {!status?.actualStartTime ? (
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <SelectTrigger
                      className="w-48"
                      data-testid="select-phase-transition"
                    >
                      <SelectValue placeholder="Change phase..." />
                    </SelectTrigger>
                    <SelectContent>
                      {allowedTransitions.map((phaseName) => {
                        const phase = COORDINATION_PHASES.find(
                          (p) => p.name === phaseName,
                        );
                        return (
                          <SelectItem
                            key={phaseName}
                            value={phaseName}
                            data-testid={`option-phase-${phaseName}`}
                          >
                            {phase?.displayName}
                          </SelectItem>
                        );
