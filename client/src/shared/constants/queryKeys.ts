...
export const queryKeys = {
  ...
  communities: {
    ...
    members: (id: string) => [...queryKeys.communities.all, id, 'members'] as const,
  ...
  events: {
    ...
    byUserCommunity: (userId: string, communityId: string) => 
      [...queryKeys.events.all, 'user', userId, 'community', communityId] as const,
...
export const queryUtils = {
   ...
   */
  prefetchRelatedData: async (queryClient: any, type: 'user' | 'community' | 'event', id: string) => {
    switch (type) {
      case 'user':
        ...
        await Promise.all([
          ...
          queryClient.prefetchQuery({ queryKey: queryKeys.messaging.notifications(id) }),
        ]);
        break;
      case 'community':
        // Prefetch community members and stats
        await Promise.all([
          queryClient.prefetchQuery({ queryKey: queryKeys.communities.members(id) }),
          queryClient.prefetchQuery({ queryKey: queryKeys.communities.stats(id) }),
        ]);
        break;
      case 'event':
        // Prefetch event details
        await queryClient.prefetchQuery({ queryKey: queryKeys.events.detail(id) });
        break;
    }
  },
};