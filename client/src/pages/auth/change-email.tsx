...
export default function ChangeEmail() {
  ...
  const [confirmationStatus, setConfirmationStatus] = useState<'idle' | 'success' | 'error' | 'expired'>('idle');
  ...
  const confirmEmailChange = async (verificationToken: string) => {
    ...
    try {
      const response = await fetch(`/api/email/confirm-email-change?token=${encodeURIComponent(verificationToken)}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        if (data.message?.includes('expired') || data.message?.includes('invalid')) {
          ...
          setConfirmationError(data.message || 'Email change confirmation failed.');
        }
      }
    } catch (err) {
        ...
        <div className="w-full max-w-md">
          <Card data-testid="card-email-confirmation">
            ...
            <CardContent className="space-y-4">
              {isConfirming && (
                <div className="flex items-center justify-center py-8" data-testid="loader-confirming">
              ...
              {confirmationStatus === 'success' && (
                <Alert data-testid="alert-confirmation-success">
                  ...
                  <AlertDescription>
                    Your email address has been updated successfully! Redirecting to dashboard...
                  </AlertDescription>
                </Alert>
              )}

              {confirmationStatus === 'error' && (
                <Alert variant="destructive" data-testid="alert-confirmation-error">
              ...
              {confirmationStatus === 'expired' && (
                <Alert variant="destructive" data-testid="alert-confirmation-expired">
                  <XCircle className="h-4 w-4" />
                  <AlertDescription>
                    This email change link has expired or is invalid. Please request a new email change.
                  </AlertDescription>
                </Alert>
              )}
            ...
            {pendingRequest ? (
              ...
              <div className="space-y-4">
                <Alert data-testid="alert-pending-request">
                  ...
                  <AlertDescription>
                    Email change verification sent to <strong>{pendingRequest.newEmail}</strong>.
              ...
              <form onSubmit={initiateEmailChange} className="space-y-4">
                <div className="space-y-2">
                  ...
                  />
                  {formErrors.newEmail && (
                    <p className="text-sm text-destructive" data-testid="error-new-email">
                      {formErrors.newEmail}
                    </p>
                  )}
                </div>
